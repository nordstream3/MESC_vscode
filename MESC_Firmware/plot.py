...
# Regular expressions
#osc52_pattern = re.compile(rb'\033]52;c;(.*?)\a')
osc52_pattern = re.compile(rb'\x1b\]52;c;(.*?)\x07')
label_pattern = re.compile(rb'\b([a-zA-Z_][a-zA-Z0-9_]*)\b(?=\s*[fuiFUIHh][+-]?\d*\.?\d*)')
type_pattern = re.compile(rb'\b([fuiFUIHh])([+-]?\d*\.?\d*)?\b')
bits_pattern = re.compile(rb'<header (\d+)bits>')

bits_and_size_pattern = re.compile(rb'bits=(\d+)\s+size=(\d+)')
...
def getHeader(osc52_str):
    ...
    #osc52_str = osc52_chunk.decode('utf-8', errors='ignore')
    print(osc52_str)

    # Extract bits and size
    match = re.search(bits_and_size_pattern, osc52_str)
    if match:
        bits = int(match.group(1))
        osc52_size = int(match.group(2))
    else:
...
def follow_osc52_chunks(file_path, chunk_size=1024):
    ...
    with open(file_path, "rb") as file:  # Open file in binary mode
        ...
        while True:
            ...
            while True:
                match = osc52_pattern.search(buffer)
                if not match:
                    break
                ...
                if b'<header ' in coded_osc52_data:
                    getHeader(coded_osc52_data)
                    osc52_pattern = re.compile(rb'\x1b\]52;c;(.{%d})\x07' % osc52_size)
                    #osc52_pattern = re.compile(rb'\033]52;c;(.{%d})\a' % osc52_size)
                    buffer = buffer[match.end():]  # Remove processed data from the buffer
                    continue

                buffer = buffer[match.end():]  # Remove processed data from the buffer

                yield coded_osc52_data

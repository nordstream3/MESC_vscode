...
void populate_vars(){
	...
	TERM_addVar(mtr[0].FOC.FOCAngle					, 0			, 65535		, "FOCangle"	, "FOC angle now"						, VAR_ACCESS_RW	, NULL		, &TERM_varList);
	...
	TERM_addVar(MESC_all_errors						, 0	, (uint32_t)HUGE_VAL	, "error_all"	, "All errors encountered"				, VAR_ACCESS_R	, NULL		, &TERM_varList);
	...
	TERM_addVar(mtr[0].FOC.speed_req				, -10000.0f	, 10000.0f	, "speed_req"	, "Speed request"						, VAR_ACCESS_RW	, NULL		, &TERM_varList);
	...
	TermVariableDescriptor * desc;
		...
		desc = TERM_addVar(MESC_errors          	, 0 , (uint32_t)HUGE_VAL  , "error" 	, "System errors now"       					, VAR_ACCESS_TR  , NULL			, &TERM_varList);
		TERM_setFlag(desc, FLAG_TELEMETRY_ON);

		desc = TERM_addVar(mtr[0].FOC.Vdq.q     	, -4096.0f , 4096.0f  	, "Vq"    	, "FOCVdq_q"     							, VAR_ACCESS_TR  , NULL         , &TERM_varList);
		TERM_setFlag(desc, FLAG_TELEMETRY_ON);

		desc = TERM_addVar(mtr[0].FOC.Vdq.d     	, -4096.0f , 4096.0f  	, "Vd"    , "FOCVdq_d"     							, VAR_ACCESS_TR  , NULL         , &TERM_varList);
		TERM_setFlag(desc, FLAG_TELEMETRY_ON);
